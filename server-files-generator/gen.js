...
var gen_packages_json = function() {
  var packages_json={};
  var packages_array=[];
  ...
  for (var i = packages.length-1; i >= 0; i--) {
    ...
    var pack_mtime = fs.statSync(config.packages_path+pack).mtime;
    console.log(pack_mtime);
    ...
    if (pack.indexOf(".ydk")>0) {
      ...
      fs.utimesSync(config.base_path+"deck\\"+pack, NaN, pack_mtime);
...
var gen_pre_json = function() {
  ...
  var packages = fs.readdirSync(config.pre_packages_path);
  for (var i = packages.length-1; i >= 0; i--) {
    var pack = packages[i];
    var packname = pack.replace(".ydk", "").replace(".txt", "");
    
    var pack_text = fs.readFileSync(config.pre_packages_path+pack,{encoding:"ASCII"})
    var pack_mtime = fs.statSync(config.pre_packages_path+pack).mtime;
    var pack_text_array = pack_text.split("\n");
    var pack_array = [];
    for (var j in pack_text_array) {
      var card=parseInt(pack_text_array[j]);
    ...
    if (pack.indexOf(".ydk")>0) {
      fs.writeFileSync(config.pre_path+"deck\\"+pack, pack_text);
      fs.utimesSync(config.pre_path+"deck\\"+pack, NaN, pack_mtime);
      args.push("-i!deck\\"+pack);
    }
    ...
    spawnSync(config.sevenzip_exe, args, { cwd: config.pre_path, env: process.env });
    
    var item={};
    item.name="先行卡卡图包 "+packname;
    item.filename=packname + ".7z";
    item.filecount=files.length;
    item.filesize=fs.statSync(config.pre_path+packname+".7z").size;
    item.files=files;
    packages_array.push(item);
  }
